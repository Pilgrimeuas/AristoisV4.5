...
void penisRotate(glm::mat4& matrix, float upper) {
	matrix = glm::translate<float>(matrix, glm::vec3(-0.5F, upper, 0.0F));
	matrix = glm::rotate<float>(matrix, 32.5F, glm::vec3(0.1F, 1.1F, 0.0F));
	matrix = glm::rotate<float>(matrix, -80.4F, glm::vec3(1.5F, 0.0F, 0.0F));
	matrix = glm::rotate<float>(matrix, 60.5F, glm::vec3(0.0F, 1.0F, 0.1F));
}
			...
			if (animations->isEnabled()) {
				...
				if (animations->shouldBlock && g_Data.canUseMoveKeys() && g_Data.isInGame() && !clickGUI->isEnabled()) {
					...
					if (animations->mode.getSelectedValue() == 5) {
						matrix = glm::translate<float>(matrix, glm::vec3(0.42222223281, 0.0, -0.26666666269302368));
						matrix = glm::translate<float>(matrix, glm::vec3(0.60f, 0.20f, -0.3f));
						penisRotate(matrix, 0.2f);
	...
	DrawUtils::setCtx(renderCtx, dat);
	{
...

		{
			...
			if (strcmp(screenName.c_str(), "start_screen") == 0) {
				...
				DrawUtils::drawSystemImage("textures/ui/promo_creeper.png", vec2_t(200, 200), vec2_t(100, 100), vec2_t(0.f, 0.f), vec2_t(1.f, 1.f), MC_Color(255, 255, 255,0));
			}*/
...

				{
					...
					if (shouldRenderWatermark) {
						...
						static std::string name = "Aristois";
#ifdef _DEBUG
						static std::string version = "V3";
#elif defined _BETA
		...
		if (notificationsMod->mode.getSelectedValue() == 0) {
			if (notificationsMod->isEnabled()) {
				...
				int index = 0;
				index++;
				int curIndex = -index * interfaceMod->spacing;
				auto interfaceColor = ColorUtil::interfaceColor(curIndex);
				float yPos = windowSize.y - 14;  // Subtract 2 from yPos
				for (auto& notification : notifications) {
					...
					vec4_t rect = vec4_t(
						windowSize.x - margin - fullTextLength - 2 - borderPadding * 2 - 7,
						yPos - margin - textHeight - 4,
						windowSize.x - margin + borderPadding - 2 - 7,
						yPos - margin - 2);  // Subtract 2 from the height
					...
					if (notification->duration <= 1 && notification->duration > -1) {
						vec4_t rect2 = vec4_t(
							...
							yPos - margin);  // Subtract 2 from the height

						vec2_t textPos = vec2_t(rect2.x + 9, rect2.y + 1);
						vec2_t titlePos = vec2_t(rect2.x + 9, rect2.y + 1);
						DrawUtils::drawText(vec2_t(textPos.x, textPos.y), &textStr, MC_Color(100, 100, 100), 0.8, 1, Fonts::SMOOTH);
						auto Notif123 = moduleMgr->getModule<Notifications>();
					}
					...
					error:
						...
						DrawUtils::drawText(vec2_t(textPos.x, textPos.y), &textStr, MC_Color(255, 255, 255), 0.8, 1, Fonts::SMOOTH);
						auto Notif123 = moduleMgr->getModule<Notifications>();
						DrawUtils::fillRoundRectangle(vec4_t{rect.x + 1.5f, rect.y, rect.x + duration, rect.w + 0.5f}, MC_Color(0, 0, 0), 1);
						DrawUtils::fillRoundRectangle(vec4_t{rect.x + 1.5f, rect.y, rect.x + duration, rect.w + 0.5f}, MC_Color(interfaceColor), 1);
					}
					yPos -= margin + 32;  // Subtract 2 from the height
				}
			}
			DrawUtils::flush();
			...
			if (notificationsMod->isEnabled()) {
				...
				for (auto& notification : notifications) {
					...
					if (notification->fadeTarget == 1 && notification->duration <= 0 && notification->duration > -1)
						...
						notification->duration -= 0.74f / 45;
					const float titleTextSize = notification->fadeVal * 1;
					const float messageTextSize = notification->fadeVal * 1;
					const float titleTextHeight = DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() * titleTextSize;
					vec2_t* pos;
					...
					if (notification->duration <= 1 && notification->duration > -1) {
						...
						DrawUtils::drawText(vec2_t(textPos.x, textPos.y), &textStr, MC_Color(100, 100, 100), 0.8, 1, Fonts::SMOOTH);
						auto Notif123 = moduleMgr->getModule<Notifications>();
						DrawUtils::fillRectangleA(rect2, MC_Color(0, 0, 0, Notif123->opacity));
					}
					...
					if (notification->duration > 1) {
						...
						DrawUtils::drawText(vec2_t(textPos.x, textPos.y), &textStr, MC_Color(255, 255, 255), 0.8, 1, Fonts::SMOOTH);
						auto Notif123 = moduleMgr->getModule<Notifications>();
						DrawUtils::fillRectangleA(rect, MC_Color(0, 0, 0, Notif123->opacity));
						DrawUtils::drawBottomLine(vec4_t{rect.x + 1.5f, rect.y, rect.x + duration, rect.w + 0.5f}, MC_Color(interfaceColor), 1);
						yPos -= margin + 30;
					}
				}
				DrawUtils::flush();
	...
	if (tx != nullptr && text->getTextLength() >= 1 && tx[0] == '.') {
		...
		if (!searchResults.empty()) {
			...
			if (firstResult.shouldReplace) {
				...
				static syncShit sync = reinterpret_cast<syncShit>(FindSignature("40 53 48 83 EC ? 48 8B DA 48 8D 4C 24 ? E8 ? ? ? ? 90 48 8B 40 ? 48 8B 08 48 8B 01 48 8B D3 FF 90 ? ? ? ? 90 F0 48 FF 0D ? ? ? ? 48 8B 44 24 ? 48 85 C0 74 ? 48 83 38 ? 74 ? 80 7C 24 ? ? 74 ? F0 48 FF 0D ? ? ? ? 48 8B 4C 24 ? 48 85 C9 74 ? 80 7C 24 ? ? 74 ? E8 ? ? ? ? C6 44 24 ? ? 48 8D 4C 24 ? E8 ? ? ? ? 48 8D 4C 24 ? E8 ? ? ? ? 80 7C 24 ? ? 74 ? 48 8B 4C 24 ? E8 ? ? ? ? 90 48 83 C4 ? 5B C3 B9 ? ? ? ? E8 ? ? ? ? CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC 40 53"));
				sync(text, text);
			}
		}
		return;
...
void Hooks::chatLogHookFunc(__int64 a1, TextHolder* msg, uint32_t a2) {
	...
	if (message.getText() != nullptr && player != nullptr) {
		...
		if (sessionInfo->isEnabled()) {  // wtf bro LOL
			...
			if (textStr.find(name) != std::string::npos && textStr.find("by") != std::string::npos) sessionInfo->kills += 1;
			if (textStr.find(name) != std::string::npos && textStr.find("was brutally destroyed") != std::string::npos) sessionInfo->dead += 1;
			if (textStr.find(name) != std::string::npos && textStr.find("slain") != std::string::npos) sessionInfo->kills += 1;
			if (textStr.find(name) != std::string::npos && textStr.find("jumped from top of the empire state building") != std::string::npos) sessionInfo->dead += 1;
		}
	}
...
float Hooks::GameMode_getPickRange(C_GameMode* _this, __int64 a2, char a3) {
	...
	if (g_Data.getLocalPlayer() != nullptr) {
		...
		if (teleportModule->isEnabled())
			return 255;
		static auto sb1 = moduleMgr->getModule<PistonAura>();
		static auto sb2 = moduleMgr->getModule<CrystalAuraWTA>();
		static auto sb3 = moduleMgr->getModule<CrystalPlace>();
		static auto sb4 = moduleMgr->getModule<CevBreaker>();
		static auto sb5 = moduleMgr->getModule<AnchorAura>();
		static auto sb6 = moduleMgr->getModule<SelfTrap>();
		static auto sb7 = moduleMgr->getModule<Surround>();
		static auto sb8 = moduleMgr->getModule<AnvilAura>();
		static auto sb9 = moduleMgr->getModule<AutoTrap>();
		static auto sb10 = moduleMgr->getModule<Burrow>();
		if (sb1->isEnabled() || sb2->isEnabled() || sb3->isEnabled() || sb4->isEnabled() || sb5->isEnabled() || sb6->isEnabled() || sb7->isEnabled())
			return 0;
		if (sb8->isEnabled() || sb9->isEnabled() || sb10->isEnabled())
			return 0;
	}
