...
void AutoCrystal::onPreTick(C_GameMode* act) {
	if (g_Data.getLocalPlayer() != nullptr) {
		...
		if (!g_Data.getLocalPlayer()->isAlive())
			return;
		if (stopOnInv) {
			if (!g_Data.canUseMoveKeys())  // TODO: Find out if 2b2e.org kicks you for placing in inventory
				return;
		} else {
			if (!g_Data.canUseMoveKeys())  // TODO: Find out if 2b2e.org kicks you for placing in inventory
				false;
		...
		if (!g_Data.isRightClickDown() && rightClick) {
			if (previousSlot != -1) {
...
void AutoCrystal::onTick(C_GameMode* gm) {
	if (g_Data.getCGameMode() == nullptr || g_Data.getLocalPlayer() == nullptr)
		return;
	...
	for (auto it = toPlace.begin(); it != toPlace.end(); ++it) {
		...
		g_Data.getCGameMode()->buildBlock(it._Ptr, 0, true);
...
void AutoCrystal::onEnable() {
	...
	case rotations_test:
		g_Data.getCGameMode()->player->pitch = -90;
	}
...
void AutoCrystal::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	if (!g_Data.isInGame()) {
		...
		if (flash)
			setEnabled(false);
	} else if (renderPlaced && g_Data.canUseMoveKeys()) {
		if (renderCrystal) {
			DrawUtils::setColor(1.f,0.f,0.f,1.f);
			DrawUtils::drawBox(latestCrystal.pos, latestCrystal.pos.add(1, 1, 1), 1.f, false);
			if (renderdmg && renderCrystal) {
				vec2_t textPos = DrawUtils::worldToScreen(latestCrystal.pos.add(0.5f, 0.5f, 0.5f));
				...
				DrawUtils::drawText(textPos, &text, MC_Color(1.f, 1.f, 1.f), 0.8f, 1.f, Fonts::SMOOTH);
			}
		}
	}
}
...
bool AutoCrystal::doPlace(std::vector<C_Entity*> targetList, std::vector<C_Entity*> allEnts, C_GameMode* gm) {
	...
	if (blocks.empty()) {
		...
		// https://www.mathsisfun.com/geometry/pythagoras-3d.html sort = a12 + a22 + ¡Ì(x2 + y2 + z2) + an2 ¡Ì(a12 + a22 + ¡Ì(x2 + y2 + z2) + an2) / 4.f
