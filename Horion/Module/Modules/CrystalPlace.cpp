...
#include<vector>
CrystalPlace::CrystalPlace() : IModule(0, Category::CPVP, "AutoCrystal For CPVP.ASIA") {
	registerIntSetting("Delay", &this->delay, this->delay, 0, 25);
	switchType = SettingEnum(this)
					 .addEntry(EnumEntry("None", 0))
					 .addEntry(EnumEntry("Client", 1))
					 .addEntry(EnumEntry("Flash", 2))
					 .addEntry(EnumEntry("Spoof", 3));
	registerEnumSetting("SwitchType", &this->switchType, 0);
	...
	registerFloatSetting("MinimumHealth", &this->minHealth, this->minHealth, 0.f, 20.f);
	registerFloatSetting("MaxSelfDamage", &this->maxSelfDmg, this->maxSelfDmg, 0.f, 20.f);
	registerFloatSetting("MinEnemDamage", &this->minEnemDmg, this->minEnemDmg, 0.f, 20.f);
	registerBoolSetting("AntiWeak)", &this->antiWeakness, this->antiWeakness);
	...
	registerEnumSetting("CalculateDamage", &this->calcDmgType, 0);
	registerBoolSetting("SafeMod", &this->safetyFirst, this->safetyFirst);
	...
	registerFloatSetting("FPThreshold", &this->fpThresh, this->fpThresh, 0.f, 20.f);
	registerFloatSetting("FPMinimumDmg", &this->dmgAtThresh, this->dmgAtThresh, 0.f, 20.f);
	registerFloatSetting("Opacity", &this->Opacity, 1, 0.f, 1.f);  // Opacity NEW!!!
	...
	registerIntSetting("BreakDelay (ticks)", &this->breakdelay, this->breakdelay, 0, 8);
	registerBoolSetting("Breakretardbreak", &this->retardBreak, this->retardBreak);
	...
	registerFloatSetting("BThru Walls", &this->breakWalls, this->breakWalls, 0, 10);
	registerFloatSetting("BPost Walls", &this->postBWalls, this->postBWalls, 0.f, 10.f);

	registerFloatSetting("BMinimum Health", &this->breakHealth, this->breakHealth, 0.f, 20.f);
	registerFloatSetting("BMax Self Dmg", &this->BmaxSelfDmg, this->BmaxSelfDmg, 0.f, 20.f);

	registerBoolSetting("BreakRender", &this->renderBreaking, this->renderBreaking);

	rotateBreak = SettingEnum(this)
					  .addEntry(EnumEntry("None", 0))
					  .addEntry(EnumEntry("Client", 1))
					  .addEntry(EnumEntry("Silent", 2));
	registerEnumSetting("BreakRotations", &this->rotateBreak, 0);
...
bool crystalFinder(C_Entity* curEnt, bool isRegularEntity) {
	...
	if (dist <= moduleMgr->getModule<CrystalPlace>()->breakRange) {
...
void CrystalPlace::onTick(C_GameMode* gm) {
	...
	if (ctr >= delay) {
		...
		if (g_Data.getLocalPlayer()->getSelectedItemId() == 637 && !currentlySwitchingHotbar || switchType.GetSelectedEntry().GetValue() == 3) {
			...
			if (switchType.GetSelectedEntry().GetValue() == 3) {
				stopSpoofing(); /*				__int64 id = *g_Data.getLocalPlayer()->getUniqueId();
				C_PlayerInventoryProxy* supplies = g_Data.getLocalPlayer()->getSupplies();
				if (g_Data.getLocalPlayer()->isAlive() && g_Data.getLocalPlayer()->getSelectedItemId() != 0 && supplies != nullptr && supplies->inventory != nullptr && supplies->selectedHotbarSlot != -1 && supplies->inventory->getItemStack(supplies->selectedHotbarSlot) != nullptr && supplies->inventory->getItemStack(supplies->selectedHotbarSlot)->getItem() != nullptr && supplies->inventory->getItemStack(supplies->selectedHotbarSlot)->item != nullptr && !supplies->inventory->getItemStack(supplies->selectedHotbarSlot)->getItem()->isWeapon() && !supplies->inventory->getItemStack(supplies->selectedHotbarSlot)->getItem()->isShooter() && switchType.GetSelectedEntry().GetValue() == 3) {
		...
		g_Data.forEachEntity([](C_Entity* ent, bool b) {
			if (ent->getEntityTypeId() != 71)
				return;
			if (g_Data.getLocalPlayer()->getPos()->dist(*ent->getPos()) < 10) {
...
void CrystalPlace::onLevelRender() {
	...
	if (!g_Data.canUseMoveKeys() ||  !g_Data.getLocalPlayer()->isAlive()) 
		return;
	...
	if (renderPlacing) {
		for (CrystalInfo& indivCrystal : CJTWDPlaceArr) {
			...
			//pos = start->lerp(end, te);
			//std::vector<vec3_t> posList;
			//posList.reserve(56);
			///	posList.push_back(pos);
			//DrawUtils::setColor(rcolors[0 & 1], rcolors[2], rcolors[0 & 2], Opacity);
			DrawUtils::drawBox3dFilled(placeLoc.sub(0, 1, 0), placeLoc.add(1, 0, 1), 1.f, false);
	...
	if (!crystalList.empty() && renderPlacing && g_Data.isInGame()) {
		anim++;
		...
		vec3_t* start = crystalList[0]->getPosOld();
		vec3_t* end = crystalList[0]->getPos();
		...
		DrawUtils::drawLineStrip3D(posList, 5);
			...
			for (double phi = 0.; phi < 2 * PI; phi += PI / 100.) // Azimuth [0, 2PI]
			{
				for (double theta = 0.; theta < PI; theta += PI / 100.) // Elevation [0, PI]
				{
					...
					vec3_t curPos = crystalList[0]->eyePos0;
					curPos.y += 0.5f;
...
void CrystalPlace::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (renderPlacing) {
		for (CrystalInfo& indivCrystal : CJTWDPlaceArr) {
			...
			DrawUtils::setColor(100.f, .75f, 100.f, sb / 100.f);
			// DrawUtils::drawBox3dFilled(placeLoc.sub(0, 1, 0), placeLoc.add(1, 0, 1), 1.f, false, true);
			DrawUtils::drawBox(placeLoc.sub(0, 0, 0), placeLoc.add(0, 0.5f, 0), 1.f, true);
...
void CrystalPlace::sortCrystals() {
