...
void pvpResources::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (!text) {
		...
		if (isThereXp()) {
			...
			std::string yikes = std::to_string(countItem(508));
		...
		//xp bottles
		std::string xp = std::to_string(countItem(508));
		DrawUtils::drawText(vec2_t(pvpX, pvpY), &std::string("Xp Bottles: " + xp), MC_Color(160, 255, 0), scale);
		//totems
