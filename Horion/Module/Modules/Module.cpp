...
void IModule::setEnabled(bool enabled) {
	if (this->enabled != enabled) {
		...
		if (player == nullptr ||
			clickGUI->isEnabled() || strcmp(screenName.c_str(), "start_screen") == 0)
			shouldShow = false;
		...
		if (notifications->showToggle && shouldShow && !((GameData::isKeyDown('L') && GameData::isKeyDown(VK_CONTROL)) || GameData::shouldTerminate())) {
			if (notifications->isEnabled()) {
				...
				auto notification = g_Data.addNotification(noftitle, std::string(std::string(state) + " " + this->getRawModuleName()));
				notification->duration = 3.f;
				if (notifications->color) {
					if (enabled) {
						notification->colorR = 42;
						notification->colorG = 113;
						notification->colorB = 47;
					} else {
						notification->colorR = 127;
						notification->colorG = 34;
						notification->colorB = 25;
					}
				}
			}
		}
