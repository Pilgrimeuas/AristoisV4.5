...
ArrayList::ArrayList() : IModule(0, Category::VISUAL, "Displays enabled modules") {
	...
	mode.addEntry(EnumEntry("Full", 1));
	mode.addEntry(EnumEntry("Split", 2));
	mode.addEntry(EnumEntry("Bar", 3));
	mode.addEntry(EnumEntry("Right", 4));
	mode.addEntry(EnumEntry("None", 5));
	registerEnumSetting("Animation", &animation, 1);
	animation.addEntry(EnumEntry("Normal", 0));
	animation.addEntry(EnumEntry("Smooth", 1));
...
void ArrayList::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	/*	auto lock = moduleMgr->lockModuleList();
		...
		for (std::set<IModuleContainer>::iterator mod = modContainerList.begin(); mod != modContainerList.end(); ++mod) {
			if (!mod->shouldRender) continue;
			index++;
			int curIndex = -index * interfaceMod->spacing;

			if (invert)
				selectableSurface.y -= 10.2f;
			else
				selectableSurface.w += 10.2f;
...
#pragma endregion
			...
			auto interfaceColortwo = ColorUtil::interfaceColortwo(2);
			if (arraycoloropa > 0) DrawUtils::fillRectangleA(rectPos, MC_Color(interfaceColortwo));
			if (opacity > 0) DrawUtils::fillRectangleA(rectPos, MC_Color(0, 0, 0, newopa));
			...
			case 0:
				...
				DrawUtils::drawText(textPos, &textStr, MC_Color(interfaceColor), textSize, 1.f, Fonts::SMOOTH);
				break;
			case 1:
				DrawUtils::fillRectangleA(leftRect, MC_Color(interfaceColor));
				if (mod->enabled) DrawUtils::fillRectangleA(underline, MC_Color(interfaceColor));
			...
			case 3:
				...
				break;
			case 4:
				if (index == 1) DrawUtils::fillRectangleA(topLine2, MC_Color(0, 0, 0, newopa));
