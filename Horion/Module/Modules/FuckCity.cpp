...
FuckCity::FuckCity() : IModule(0x0, Category::COMBAT, "Attacks entities around you automatically") {
...
void FuckCity::onTick(C_GameMode* gm) {
			...
			if (!isSideEmpty(targetListXD[0])) {
				...
				for (auto& a : around) {
					...
					for (auto checkThis : around) {
						...
						} else if (g_Data.getLocalPlayer()->region->getBlock(vec3_ti(calcSides))->toLegacy()->blockId == 49) {
							sidesBlocksOnly.push_back(calcSides);
						}
					}
					for (auto checkThis : around) {
						auto calcSides = rndOffPlayerOn.add(checkThis);
						if (g_Data.getLocalPlayer()->region->getBlock(vec3_ti(calcSides))->toLegacy()->blockId == 49) {
							sidesBlocksOnly.push_back(calcSides);
						}
					}
				}
				...
				if (gm->player->region->getBlock(pos)->toLegacy()->blockId == 49) {
					if (g_Data.canUseMoveKeys() && !moduleMgr->getModule<ClickGuiMod>()->isEnabled()) {
						C_PlayerInventoryProxy* supplies = g_Data.getLocalPlayer()->getSupplies();
						C_Inventory* inv = supplies->inventory;
						...
						if (hasClicked) {
							if (gm->player->region->getBlock(pos)->toLegacy()->blockId == 49 && !fastbreak) {
								odelays++;
								gm->player->level->rayHitType = 3;
								gm->player->level->rayHitVec = pos.toVec3t();
								if (odelays >= 40 + minetime) {
									for (int n = 0; n < 9; n++) {
										C_ItemStack* stack = inv->getItemStack(n);
										if (stack->item != nullptr) {
											float currentDamage = stack->getItem()->getAttackDamage();
								...
								if (odelays >= 40 + minetime + swaptime) {
									fastbreak = true;
								}
							}
...
void FuckCity::onSendPacket(C_Packet* packet) {
}
void FuckCity::onPreRender(C_MinecraftUIRenderContext* context) {
	{
		float rentimer = 0;
		if (odelays != 0 && odelays <= 46) rentimer = (float)(odelays / 46.f);
		if (odelays >= 46) rentimer = 1;
