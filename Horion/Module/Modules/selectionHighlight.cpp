...
selectionHighlight::selectionHighlight() : IModule(0x0, Category::VISUAL, "Custom selection box!") {
	...
	registerBoolSetting("XYZ Display", &this->xyz, this->xyz);
...
void selectionHighlight::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (!(g_Data.getLocalPlayer() == nullptr || !GameData::canUseMoveKeys())) {
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			...
			auto pStruct = g_Data.getClientInstance()->getLevel();
			...
			if (selectRainbow) {
				...
				if (rayType == 0) {
					...
					DrawUtils::drawBox(hLower, hUpper, thickness, doOutline);
					if (xyz) {
						DrawUtils::setColor(0.f, 0.f, 1.f, opacityVal);
						DrawUtils::drawX(hLower, hUpper, thickness, doOutline);
						DrawUtils::setColor(0.f, 1.f, 0.f, opacityVal);
						DrawUtils::drawY(hLower, hUpper, thickness, doOutline);
						DrawUtils::setColor(1.f, 0.f, 0.f, opacityVal);
						DrawUtils::drawZ(hLower, hUpper, thickness, doOutline);
					}
				}
