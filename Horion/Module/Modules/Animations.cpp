...
Animations::Animations() : IModule(0, Category::VISUAL, "Changes the swing/hitting animation") {
	...
	mode.addEntry(EnumEntry("1.7", 1));
	mode.addEntry(EnumEntry("Slide", 2));
	mode.addEntry(EnumEntry("Old", 3));
	mode.addEntry(EnumEntry("Spin", 4));
	// registerBoolSetting("SmoothSwing", &smoothSwing, smoothSwing);
	registerEnumSetting("Swing", &type, 0);
	type.addEntry(EnumEntry("None", 0));
	type.addEntry(EnumEntry("Flux", 1));
	type.addEntry(EnumEntry("Tap", 2));
	type.addEntry(EnumEntry("Exhi", 3));
	// registerBoolSetting("SlowSwing", &slowSwing, slowSwing);
...
void Animations::onPlayerTick(C_Player* plr) {
	...
	if (mode.getSelectedValue() == 5 && shouldBlock) {
