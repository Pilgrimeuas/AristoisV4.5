...
const char* SilentLook::getModuleName() {
	return "SilentLook";
}
vec2_t mouseEnd = {0, 0};
void SilentLook::onEnable() {
...
void SilentLook::onPostRender(C_MinecraftUIRenderContext* mode) {
...
void SilentLook::onTick(C_GameMode* mode) {
	if(this->resetViewTick == 0){
		this->redirectMouse = false;
		this->resetViewTick = -1;
