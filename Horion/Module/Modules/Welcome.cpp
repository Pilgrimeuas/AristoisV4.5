...
Welcome::Welcome() : IModule(0x0, Category::HUD, "Welcomes you at the top of the screen") {
	registerEnumSetting("Text Modes", &TextMode, 0);
	TextMode = SettingEnum(this)
				   .addEntry(EnumEntry("Welcome", 0))
				   .addEntry(EnumEntry("Aristois", 1))
				   .addEntry(EnumEntry("User", 2))
				   .addEntry(EnumEntry("Hello", 3));
...
void Welcome::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.isInGame() && GameData::canUseMoveKeys()) {
		...
		std::string playerName = std::string(g_Data.getLocalPlayer()->getNameTag()->getText()) + "¡ìr";
		...
		case 1:
			name = "Welcome to Aristois," + playerName;
			break;

		case 2:
			name = "Aristois user " + playerName;
			break;
