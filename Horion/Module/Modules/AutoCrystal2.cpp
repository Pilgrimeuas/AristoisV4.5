#include "AutoCrystal2.h"
AutoCrystal2::AutoCrystal2() : IModule(0x0, Category::COMBAT, "Automatically place a crystal at the enemy's location and exploade it speedily.") {
...
void findEntityK3R(C_Entity* currentEntity, bool isRegularEntity) {
	static auto AutoCrystalMod = moduleMgr->getModule<AutoCrystal2>();
...
bool checkTargCollision12(vec3_t* block, C_Entity* ent) {
	...
	corners.clear();
	static auto AutoCrystalMod = moduleMgr->getModule<AutoCrystal2>();
	int dmgMode5 = AutoCrystalMod->dmgEnum.GetSelectedEntry().GetValue();

	if (dmgMode5 == AutoCrystalMod->dmg_2b2e) {
...
std::vector<vec3_t*> getGucciPlacement21(C_Entity* ent) {
	static auto AutoCrystalMod = moduleMgr->getModule<AutoCrystal2>();
	int dmgMode2 = AutoCrystalMod->dmgEnum.GetSelectedEntry().GetValue();
	...
	for (auto checkThis : SurChecker) {
		for (auto checkThis2 : SurChecker2) {
			...
			if (g_Data.getLocalPlayer()->region->getBlock(vec3_ti(calcSides))->toLegacy()->blockId == 0 && dmgMode2 == AutoCrystalMod->dmg_vanilla ? g_Data.getLocalPlayer()->region->getBlock(vec3_ti(calcSides2))->toLegacy()->blockId == 0 : g_Data.getLocalPlayer()->region->getBlock(vec3_ti(calcSides))->toLegacy()->blockId == 0) {
				sendBack = true;
			}
		}
	}
...
void AutoCrystal2::onTick(C_GameMode* gm) {
						...
						if (g_Data.getLocalPlayer()->getSelectedItem()->getItem()->isCrystal()) {
							...
							for (auto place : gucciPositions) {
								...
								if (!(targetList7[0]->damageTime > 1 && HurtTime)) {
									gm->buildBlock(&vec3_ti(place->x, place->y - 1, place->z), 1,3);
	...
	g_Data.forEachEntity([](C_Entity* ent, bool b) {
		...
		int range = moduleMgr->getModule<AutoCrystal2>()->range;
...
void AutoCrystal2::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
