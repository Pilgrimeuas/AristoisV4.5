...
void CommandMgr::initCommands() {
	...
	commandList.push_back(new AutoTextCommand());
...
void CommandMgr::execute(char* message) {
	if (message != nullptr) {
		...
		g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%sAristois%s] %sCommand '%s' could not be found!", GOLD, WHITE, RED, cmd.c_str());
	}
}
