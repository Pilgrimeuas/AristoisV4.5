...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessageF("[%sAristois%s] %sCouldn't find player: %s!", GOLD, WHITE, RED, nameOfPlayer.c_str());
		return true;
	}
	g_Data.getLocalPlayer()->setPos(pos);
	clientMessageF("[%sAristois%s] %sTeleported to %s", GOLD, WHITE, GREEN, playerName.c_str());
	return true;
}
